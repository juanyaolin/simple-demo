<?php
    ...
    function doSomething($todoList, $id, $callback)  {
        foreach ($todoList as $index => $todo) {
            if ($todo['id'] === $id) {
                $todoList[$index] = $callback($todo);
            }
        }
    ...
    if(isset($_POST["act"])){
        if($_POST["act"] === 'clear'){
            ...
            doSomething($todoList, $id, function ($todo) {
                $todo['isEdit'] = true;

                return $todo;
            });

            // $newItem = find($todoList, $id);
            // $newItem["isEdit"] = true;
            // updateToDoList($id, $newItem, $todoList);
        }else if(explode("-" ,$_POST["act"])[0] === 'check'){
            $id = explode("-" ,$_POST["act"])[1];

            doSomething($todoList, $id, function ($todo) use ($id) {
                $todo["isEdit"] = false;
                $todo["content"] = $_POST["newContent-$id"];
            ...
            doSomething($todoList, $id, function ($todo) use ($isChecked) {
        ...
        <form class="to-do-container" action="to_do_list.php" method="post">
            <div class="to-do-header">
                <div>
                    ...
                    <p><?php echo count($_SESSION["todoList"] ?? []) ?> tasks</p>
                </div>
            ...
            <div class="form-header" action="to_do_list.php" method="post" id="to_do_form">
                ...
                <input type="hidden" name="created_at"
                    value="<?php echo date_format(date_create("now", timezone_open("Asia/Taipei")),"Y/m/d H:i:s"); ?>"
                />
                            ...
                            <li class="to-do-item">
                                <?php if ($listItem["isEdit"]) { ?>
                                    ...
                                    <label class="label-button" for="form-check-<?php echo $listItem["id"]; ?>">
                                        ...
                                        <input
                                            class="hidden"
                                            id="form-check-<?php echo $listItem["id"]; ?>"
                                            type="submit"
                                            name="act"
                                            value="check-<?php echo $listItem["id"]; ?>"
                                        />
                                    </label>
                                <?php } else { ?>
                                    <!-- 非編輯狀態 -->
                                    <label for="form-isChecked-<?php echo $listItem["id"]; ?>" class='checkbox-container'>
                                        <!-- checkbox 表示是否完成 -->
                                        <input
                                            class="hidden <?php echo $listItem['isChecked'] ? 'checked' : '' ?>"
                                            id="form-isChecked-<?php echo $listItem["id"]; ?>"
                                            type="submit"
                                            name="act"
                                            value="isChecked-<?php echo $listItem["id"]; ?>-<?php echo !$listItem['isChecked'] ?>"
                                        />
                                    ...
                                    <label class="label-button" for="form-edit-<?php echo $listItem["id"]; ?>">
                                        ...
                                        <input
                                            class="hidden"
                                            id="form-edit-<?php echo $listItem["id"]; ?>"
                                            type="submit"
                                            name="act"
                                            value="edit-<?php echo $listItem["id"]; ?>"
                                        />
                                    </label>
                                    <label class="label-button" for="form-delete-<?php echo $listItem["id"]; ?>">
                                        ...
                                        <input
                                            class="hidden"
                                            id="form-delete-<?php echo $listItem["id"]; ?>"
                                            type="submit"
                                            name="act"
                                            value="delete-<?php echo $listItem["id"]; ?>"
                                        />
                                    </label>
                                <?php } ?>
                            </li>
